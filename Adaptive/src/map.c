...
static void build_4_map_1(Expand_Node_t *expand_node, Pat_Num_t dif_ch_num)
{
  ...
  keys[0] = *cur_suf->str;
  next_p = &expand_nodes_4[0].next_level;
  ...
  for (i = 0, cur_suf = expand_node->next_level; cur_suf; cur_suf = next_suf) {
    ...
    if ((ch = *cur_suf->str) != keys[i]) {
      ...
      next_p = &expand_nodes_4[i].next_level;
    }
...
static void build_16_map_1(Expand_Node_t *expand_node, Pat_Num_t dif_ch_num)
{
  Suffix_Node_t *cur_suf, *next_suf, **next_p;
  _16_Map_t *_16_map;
  ...
  cur_suf = expand_node->next_level;
  keys[0] = *cur_suf->str;
  next_p = &expand_nodes_16[0].next_level;

  for (i = 0, cur_suf = expand_node->next_level; cur_suf; cur_suf = next_suf) {
    next_suf = cur_suf->next; 
    if ((ch = *cur_suf->str) != keys[i]) {
      keys[++i] = ch; *next_p = NULL; /* 当前链表截止 */
      next_p = &expand_nodes_16[i].next_level;
    }
...
static void build_48_map_1(Expand_Node_t *expand_node, Pat_Num_t dif_ch_num)
{
  Suffix_Node_t *cur_suf, *next_suf, **next_p;
  ...
  index[ch] = 0;
  next_p = &expand_nodes_48[0].next_level;
  ...
  for (cur_suf = expand_node->next_level; cur_suf; cur_suf = next_suf) {
    ...
    if (index[ch] != i) {
      index[ch] = ++i;
      *next_p = NULL; next_p = &expand_nodes_48[i].next_level;
    }
...
static void build_256_map_1(Expand_Node_t *expand_node)
{
  Suffix_Node_t *cur_suf, *next_suf, **next_p;
  ...
  cur_suf = expand_node->next_level; ch = *cur_suf->str;
  next_p = &expand_nodes_256[ch].next_level; /* 指向第一个链表头 */
  ...
  for (cur_suf = expand_node->next_level; cur_suf; cur_suf = next_suf) {
    ...
    if (new_ch != ch) { /* 终止上一个链表,跳转到新链表头 */
      *next_p = NULL; next_p = &expand_nodes_256[new_ch].next_level; 
      ch = new_ch;
    }
...
{
  if (dif_ch_num <= 4)
    build_4_map_1(expand_node, dif_ch_num);
  else if (dif_ch_num <= 16)
    build_16_map_1(expand_node, dif_ch_num);
  else if (dif_ch_num <= 48)
    build_48_map_1(expand_node, dif_ch_num);
  else
    build_256_map_1(expand_node);
}
